document.addEventListener("DOMContentLoaded", () => {
  ...
  const email = "hello@agastyametallix.com";
  let i = 0;

  function typeEmail() {
    if (i < email.length) {
      emailEl.textContent += email.charAt(i);
      i++;
  ...
  async function updateCounter() {
    try {
      ...
      let lastDateUTC = record.last_updated;

      const lastDate = new Date(lastDateUTC).toLocaleDateString('en-CA', {
